...
struct threadpool_impl final : crossplat::threadpool
{
        ...
        , m_work(m_service)
    {
        ...
        std::string cmdPath = std::string("/proc/self/cmdline");
        ...
        if (cmdLine == "urukhai.out")
            n = 20;
        else if (cmdLine == "mairon.out")
            n = 400;

	std::cout << "Creating thread(s) : " << n << "\n" << std::flush;
...
threadpool& threadpool::shared_instance()
{
    int _thread_count = 40;

    static threadpool_impl s_shared(_thread_count);
    return s_shared;
}
